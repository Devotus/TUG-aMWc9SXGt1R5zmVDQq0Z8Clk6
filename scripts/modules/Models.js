(function() {
    ...
    app.factory('Manifest', ['msgHan', 'ModServices', 'Section', 'Content', 'Media',
        function(msgHan, ModServices, Section, Content, Media) {
            var defaultMedia1 = new Media('video', "na1NMqoGZ_g")
            var defaultMedia2 = new Media('image', "https://www.rockpapershotgun.com/images/13/may/tug.jpg")
            ...
            var getGameNameFromURL = function(){
                return window.location.toString().substring(window.location.toString().indexOf('hub.io/') + 7, window.location.toString().indexOf('-'))
            }

            var getModNameFromURL = function(game){
                return window.location.toString().substr(window.location.toString().indexOf(game + '-')+ (game.length + 1), window.location.toString().length-1 - (window.location.toString().indexOf(game + '-')+ (game.length + 1)))
            }
            ...
            Manifest.prototype.get = function(cb) {
                var self = this;
                var gameName = getGameNameFromURL()
                var modName = getModNameFromURL(gameName)
                ModServices.getManifest(gameName, modName, function(msg) {
                    ModServices.getAuthors(gameName, modName, function(authors){
